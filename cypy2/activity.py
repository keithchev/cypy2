...
class Activity(object):
    ...
    def from_db(cls, conn, activity_id, kind=None):
        ...
        # load the raw data
        if kind in ['raw', 'all']:
            activity.load(conn=conn, kind='raw')
    
        # load or generate the processed data
        if kind=='raw':
            activity.process()
        elif kind in ['processed', 'all']:
            activity.load(conn=conn, kind='processed')

        return activity


    def load(self, conn=None, data=None, kind='raw'):
        '''
        Load the activity's data from either raw user-provided data or a cypy2 database

        ('user-provided data' means *raw* data loaded from a local FIT file;
        by design, there is no way to load processed data from a local file/cache)

        '''

        # from a cypy2 database
        if conn is not None:
            if kind in ['raw', 'all']:
        ...
        # from local data
            ...
            if kind!='raw':
                raise ValueError('kind must be \'raw\' when loading from user-provided data')

            required_keys = ['events', 'summary', 'records']
            if set(required_keys).difference(data.keys()):
                raise ValueError('Some raw data types are missing')
            self._raw_data = data

        else:
            raise ValueError(
                'Activity.load requires either a database connection or user-provided data')
...
class LocalActivity(Activity):
    ...
    def __init__(self, fit_data, strava_metadata=None):
        ...
        # 'load' the raw data
        self.load(data=raw_data, kind='raw')
